...
#region Unity Functions
            ...
            // Dispose of the couroutines if the audio manager is deleted
            // Avoids memory leaks  
...
#region Private Functions
        ...
        // Cancels potential running jobs
        ...
        private void GenerateAudioTable() {
            foreach(AudioTrack _track in tracks) {
                foreach(AudioObject _object in _track.audio) {
                    ...
                    if (m_AudioTable.ContainsKey(_object.type)) {
                        LogWarning("You are trying to register audio [" + _object.type + "] that has already been registered.");
                    } else {
                        m_AudioTable.Add(_object.type, _track);
                        Log("Registering audio [" + _object.type + "].");
                    }    
                }
            }
        }
        ...
        private void RemoveConflictingJobs(AudioType _type) {
            ...
            // For every key value pair in job table, check if audio type is the same as the audio type currently trying to be played
            foreach(DictionaryEntry _entry in m_JobTable) {
